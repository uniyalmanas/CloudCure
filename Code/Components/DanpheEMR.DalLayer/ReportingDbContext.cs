...
namespace DanpheEMR.DalLayer
{
    public class ReportingDbContext : DbContext
    {
        ...
        public ReportingDbContext(string conn) : base()
        {
            _connString = conn;
            connStr = conn;
        }
        ...
        private DataSet GetPatientBillHistory2(DateTime? FromDate, DateTime? ToDate, string PatientCode, string connString)
        {
            ...
            var dbTimeout = context.Database.GetCommandTimeout();
            cmd.CommandTimeout = dbTimeout ?? 180;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "SP_Report_BILL_PatientBillHistory";
            cmd.Parameters.AddSafe(new SqlParameter("@FromDate", FromDate));
            cmd.Parameters.AddSafe(new SqlParameter("@ToDate", ToDate));
            cmd.Parameters.AddSafe(new SqlParameter("@PatientCode", PatientCode));
