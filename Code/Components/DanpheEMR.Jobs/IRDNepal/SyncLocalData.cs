            ...
            using (IRDNepalDbContext dbContext = new IRDNepalDbContext(connString))
            {
                try
                {
                    ...
                    var retBillsToSync = (from ret in dbContext.BillReturnRequests
                                          ...
                                          where !ret.IsRemoteSynced
                                          select new
                                          {
                                              module_name = "billing",
                                              invoice_type = "sales-return",
                                              table_data_id = ret.BillReturnId,
                                              ...
                                              total_sales = ret.SubTotal,//REVISION Reqd.
                                              discount_amount = ret.DiscountAmount,//REVISION Reqd.
                                              taxable_sales_hst = ret.TotalAmount,//REVISION Reqd.
                                              hst = ret.TaxTotal,//REVISION Reqd.
                                              entered_by = ret.CreatedBy.ToString(),//need username here.                 
                                              credit_note_number = ret.BillReturnId,//REVISION Reqd.
                                              credit_note_date = ret.CreatedOn.ToString(),
                                              remarks = ret.Remarks,//REVISION Reqd.
                    ...
                    if (retBillsToSync != null && retBillsToSync.Count > 0)
                    {
                        using (var dbContextTransaction = dbContext.Database.BeginTransaction())
                        {
                            try
                            {
                                List<BillInvoiceReturnModel> retItemsToUpdate = new List<BillInvoiceReturnModel>();
                                ...
                                foreach (BillInvoiceReturnModel ret in retItemsToUpdate)
                                {
