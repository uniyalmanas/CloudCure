    ...
    public class VisitController : CommonController
    {
        ...
        public ActionResult PatientTodaysVisits(int patientId)
        {
            ...
            Func<object> func = () => (from visit in _visitDbContext.Visits
                                     ...
                                     //((DateTime)defaultLastDateToShow).Date
                                     && ((DateTime)visit.VisitDate).Date == ((DateTime)DateTime.Now).Date
        ...
        private object CheckExistingAppointmentOnDate(DateTime requestDate, int departmentId, int patientId, int inputProviderId)
        {
            ...
            var visitItem = _visitDbContext.Visits.Where(v => (((DateTime)v.VisitDate).Date == ((DateTime)requestDate.Date).Date) && (v.DepartmentId == departmentId) && (v.PatientId == patientId && v.BillingStatus.ToLower() != ENUM_BillingStatus.returned && v.BillingStatus.ToLower() != ENUM_BillingStatus.cancel)).AsEnumerable();
            if (isDeparmentLevelVisit == "false")
                visitItem = _visitDbContext.Visits.Where(v => (((DateTime)v.VisitDate).Date == ((DateTime)requestDate.Date).Date) && (v.PerformerId == inputProviderId) && (v.PatientId == patientId && v.BillingStatus.ToLower() != ENUM_BillingStatus.returned && v.BillingStatus.ToLower() != ENUM_BillingStatus.cancel)).AsEnumerable();
            if (visitItem.Any())
            {
                return true;
            }
        ...
        private object GetPatientVisitsWithDoctors(int patientId)
        {
            ...
            var patAllVisits = (from v in _visitDbContext.Visits
                                ...
                                && ((DateTime)v.VisitDate).Date >= ((DateTime)maxLastVisitDate).Date
        ...
        private object GetVistByStatus(string search, string status, int dayslimit)
        {
            ...
            var visitVMList = (from visit in _visitDbContext.Visits
                               ...
                               where ((visit.VisitStatus == status)
                                  && visit.VisitDate > ((DateTime)defaultLastDateToShow).Date && visit.VisitType != ENUM_VisitType.inpatient) && visit.BillingStatus != ENUM_BillingStatus.returned
