    ...
    public class BillingBL
    {
        ...
        public static int GetInvoiceNumber(string connString)
        {
            using (new TransactionScope(TransactionScopeOption.Required, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted }))
            {
                int fiscalYearId = GetFiscalYear(connString).FiscalYearId;
                DanpheEMR.DalLayer.BillingDbContext billDbContext = new DalLayer.BillingDbContext(connString);
                int invoiceNumber = billDbContext.BillingTransactions
                                      .Where(txn => txn.FiscalYearId == fiscalYearId)
                                      .Select(txn => txn.InvoiceNo)
        ...
        public static int? GetDepositReceiptNo(string connString)
        {
            ...
            using (new TransactionScope(TransactionScopeOption.Required,new TransactionOptions{IsolationLevel = IsolationLevel.ReadUncommitted}))
            {
                                    int fiscalYearId = GetFiscalYear(connString).FiscalYearId;
                                    BillingDbContext billDbContext = new BillingDbContext(connString);
                                    int? receiptNo = billDbContext.BillingDeposits
                                                      .Where(depTxn => depTxn.FiscalYearId == fiscalYearId)
                                                      .Select(depTxn => depTxn.ReceiptNo)
                                                      .OrderByDescending(x => x)
                                                      .FirstOrDefault() ?? 0;
        ...
        public static int? GetProvisionalReceiptNo(string connString)
        {
            int fiscalYearId = GetFiscalYear(connString).FiscalYearId;
            BillingDbContext billDbContext = new BillingDbContext(connString);
            int? receiptNo = billDbContext.BillingTransactionItems
                              .Where(txnItems => txnItems.ProvisionalFiscalYearId == fiscalYearId)
                              .Select(txnItems => txnItems.ProvisionalReceiptNo)
                              .OrderByDescending(x => x)
                              .FirstOrDefault() ?? 0;
