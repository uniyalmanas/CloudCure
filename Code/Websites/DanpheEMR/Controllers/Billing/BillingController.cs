...
using DanpheEMR.Core.Integrations;
...
namespace DanpheEMR.Controllers
{
    ...
    public class BillingController : CommonController
    {
        ...
        public BillingController(IOptions<MyConfiguration> _config, IPaymentGatewayService paymentGatewayService) : base(_config)
        {
        ...
        [HttpGet]
        [Route("GenerateUPIQR")]
        public async Task<IActionResult> GenerateUPIQR(double amount, string transactionId, string clinicUPIId = "curecloud@okaxis")
        {
            try
            {
                var base64Qr = await _paymentGatewayService.GenerateUPIReciptQR(amount, transactionId, clinicUPIId);
                _objResponseData.Results = base64Qr;
                _objResponseData.Status = ENUM_Danphe_HTTP_ResponseStatus.OK;
                return Ok(_objResponseData);
            }
            catch (Exception ex)
            {
                _objResponseData.Status = ENUM_Danphe_HTTP_ResponseStatus.Failed;
                _objResponseData.ErrorMessage = ex.Message;
                return BadRequest(_objResponseData);
            }
        }
        ...
        //            //                      && (((DateTime)itm.CreatedOn).Date >= startDate && ((DateTime)itm.CreatedOn).Date <= endDate)
        ...
        //            //                           where itm.BillStatus != ENUM_BillingStatus.cancel // "cancel" 
        //            //                           && itm.ReturnStatus != true
        //            //                              && (((DateTime)itm.CreatedOn).Date >= startDate.Date && ((DateTime)itm.CreatedOn).Date <= endDate.Date)
        //            //                           select new
        //            //                           {
        //            //                               Date = itm.CreatedOn,
        //            //                               ServiceDepartmentId = itm.ServiceDepartmentId,
