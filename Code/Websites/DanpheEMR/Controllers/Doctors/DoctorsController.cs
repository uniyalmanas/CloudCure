...
namespace DanpheEMR.Controllers
{
    ...
    public class DoctorsController : CommonController
    {
        ...
        //                         && (((DateTime)visit.VisitDate).Date == ((DateTime)today).Date || visit.Admission.AdmissionStatus == "admitted")
        ...
        //        //show only today's visits for the provider.. and if the visittype is inpatient then  the addmission status shouled be admitted ..Dharam 9th Sept 2017..
        //        var visitList = (from visit in dbContext.Visits.Include("Admission").Include("Patient")
        //                         where ((DateTime)visit.VisitDate).Date == ((DateTime)toDate).Date && // visit.VisitStatus == status &&
        //                       (((DateTime)visit.VisitDate).Date == ((DateTime)toDate).Date || visit.Admission.AdmissionStatus == "admitted")
        ...
        private object GetTodaysVisitList(DateTime toDate, string status, RbacUser currentUser)
        {
            if (toDate == null)
            {
                ...
                var rawVisits = (from visit in _doctorDbContext.Visits
                                 where visit.VisitStatus == status
                                 && (((DateTime)visit.VisitDate).Date == ((DateTime)today).Date || visit.Admission.AdmissionStatus == "admitted")
                                 && visit.PerformerId == providerId && visit.BillingStatus != ENUM_BillingStatus.returned
                                 select new
                                 {
                                     ...
                                     visit.Remarks,
                                     visit.ClaimCode,
                                     visit.IsSignedVisitSummary,
                                     PatientId_Pat = visit.Patient.PatientId,
                                     PatientCode_Pat = visit.Patient.PatientCode,
                                     FirstName_Pat = visit.Patient.FirstName,
                                     MiddleName_Pat = visit.Patient.MiddleName,
                                     LastName_Pat = visit.Patient.LastName,
                                     Gender_Pat = visit.Patient.Gender,
                                     Age_Pat = visit.Patient.Age,
                                     PhoneNumber_Pat = visit.Patient.PhoneNumber,
                                     HasAdmission = visit.Admission != null,
                                     PatientVisitId_Adm = visit.Admission != null ? (int?)visit.Admission.PatientVisitId : null,
                                     AdmissionStatus_Adm = visit.Admission != null ? visit.Admission.AdmissionStatus : null
                                 }).ToList();

                var visitList = rawVisits.Select(v => new VisitModel
                {
                    ...
                    IsSignedVisitSummary = v.IsSignedVisitSummary,
                    Patient = new PatientModel
                    {
                        PatientId = v.PatientId_Pat,
                        PatientCode = v.PatientCode_Pat,
                        FirstName = v.FirstName_Pat,
                        MiddleName = v.MiddleName_Pat,
                        LastName = v.LastName_Pat,
                        ShortName = v.FirstName_Pat + " " + (string.IsNullOrEmpty(v.MiddleName_Pat) ? "" : v.MiddleName_Pat + " ") + v.LastName_Pat,
                        Gender = v.Gender_Pat,
                        Age = v.Age_Pat,
                        PhoneNumber = v.PhoneNumber_Pat
                    },
                    Admission = v.HasAdmission ? new AdmissionModel
                    {
                        PatientVisitId = v.PatientVisitId_Adm.Value,
                        AdmissionStatus = v.AdmissionStatus_Adm
                    } : null
                }).OrderBy(v => v.VisitDate).ThenBy(v => v.VisitTime).ToList();
            ...
            else
            {
                ...
                var rawVisits = (from visit in _doctorDbContext.Visits
                                 where ((DateTime)visit.VisitDate).Date == ((DateTime)toDate).Date &&
                                 (((DateTime)visit.VisitDate).Date == ((DateTime)toDate).Date || visit.Admission.AdmissionStatus == "admitted")
                ...
                var groupedList = visitList.GroupBy(a => a.PerformerName)
                                 .Select(v => new { ProviderName = v.Select(p => p.PerformerName).FirstOrDefault(), visit = v }).ToList();
        ...
        private object GetPastVisitList(DateTime fromDate, DateTime toDate, RbacUser currentUser)
        {
            ...
            var rawVisits = (from visit in _doctorDbContext.Visits
                             ...
                             .Where(v => v.VisitDate.Date >= fromDate && v.VisitDate.Date <= toDate.Date && v.BillingStatus != ENUM_BillingStatus.returned)
                             .ToList();
        ...
        private object GetDoctorDepartmentVisit(DateTime fromDate, DateTime toDate, RbacUser currentUser)
        {
            ...
            var rawVisits = (from visit in _doctorDbContext.Visits
                             ...
                             .Where(v => v.VisitDate.Date >= fromDate && v.VisitDate.Date <= toDate.Date)
                             .ToList();

            var visits = rawVisits.Select(v => new VisitModel
            {
                PatientVisitId = v.PatientVisitId,
                VisitCode = v.VisitCode,
                PatientId = v.PatientId,
