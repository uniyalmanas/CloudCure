        ...
        private object GetPatientConsumptionList()
        {
            var rawList = (from con in _patientConsumptionDbContext.PatientConsumption.Where(a => a.BillingStatus == ENUM_BillingStatus.unpaid)
                           ...
                           select new
                           {
                               ...
                               ItemTotalAmount = conitm.TotalAmount,
                               ReturnTotalAmount = conretitm != null ? conretitm.TotalAmount : 0,
                               CreatedOn = conitm.CreatedOn
                           }).AsEnumerable();

            var patConsumptionList = (from r in rawList
                                      group r by new
                                      {
                                          r.PatientId,
                                          r.PatientCode,
                                      ...
                                      select new
                                      {
                                          ...
                                          TotalAmount = Math.Floor(g.Sum(x => x.ItemTotalAmount - x.ReturnTotalAmount)),
