    ...
    public class PharmacyPurchaseController : CommonController
    {
        ...
        public IActionResult GetOrders(string status, DateTime fromDate, DateTime toDate)
        {
            ...
            Func<object> func = () => (from po in _pharmacyDbContext.PHRMPurchaseOrder
                                       ...
                                       }).Where(a => ((DateTime)a.PODate).Date >= ((DateTime)fromDate).Date && ((DateTime)a.PODate).Date <= ((DateTime)toDate).Date).OrderByDescending(a => a.PurchaseOrderId).ToList();
        ...
        private object GetSuppliersLedgerInfo(DateTime fromDate, DateTime toDate)
        {
            ...
            goodReceiptReturn = (from s in _pharmacyDbContext.PHRMSupplier.Where(a => a.IsActive == true)
                                 join grret in _pharmacyDbContext.PHRMReturnToSupplier.Where(a => ((DateTime)a.ReturnDate).Date >= ((DateTime)fromDate).Date && ((DateTime)a.ReturnDate).Date <= ((DateTime)toDate).Date) on s.SupplierId equals grret.SupplierId into leftJ
                                 from lj in leftJ.DefaultIfEmpty()
                                 group new { s, lj } by new { s.SupplierId, s.SupplierName } into g
                                 select new PHRMSupplierGoodReceiptVM
                                 {
        ...
        private object GetSupplierLedgerInfo(int supplierId, DateTime fromDate, DateTime toDate)
        {
            ...
            goodReciptList = _pharmacyDbContext.PHRMGoodsReceipt.Where(a => a.IsCancel != true && a.SupplierId == supplierId && ((DateTime)a.GoodReceiptDate).Date >= ((DateTime)fromDate).Date && ((DateTime)a.GoodReceiptDate).Date <= ((DateTime)toDate).Date)
                                                    .Select(a =>
            ...
            goodReceiptReturn = _pharmacyDbContext.PHRMReturnToSupplier.Where(a => a.SupplierId == supplierId && ((DateTime)a.ReturnDate).Date >= ((DateTime)fromDate).Date && ((DateTime)a.ReturnDate).Date <= ((DateTime)toDate).Date)
