        ...
        public List<DonationVM> GetAllDonation(DateTime fromDate, DateTime toDate, int StoreId)
        {
            try
            {
                var donationDetails = (from donations in db.donations
                                       ...
                                       where store.StoreId == StoreId && donations.IsActive && ((DateTime)donations.CreatedOn).Date >= ((DateTime)fromDate).Date && ((DateTime)donations.CreatedOn).Date <= ((DateTime)toDate).Date
