...
export class BillingTransactionComponent_New {
  ...
  PaymentModeChanges($event) {
    ...
    if (this.model.PaymentMode.toLowerCase() === 'upi') {
      this.GenerateUPIQRCode(this.model.ReceivedAmount);
    } else {
      this.upiQrCodeBase64 = null;
    }
  }

  GenerateUPIQRCode(amount: number) {
    this.loadingUpiQr = true;
    this.upiQrCodeBase64 = null;
    const tempTxnId = 'TXN' + Math.floor(Math.random() * 10000000);
    this.httpClient.get<any>(`/api/Billing/GenerateUPIQR?amount=${amount}&transactionId=${tempTxnId}`)
      .subscribe(res => {
        this.loadingUpiQr = false;
        if (res && res.Status === "OK") {
          this.upiQrCodeBase64 = res.Results;
        } else {
          this.msgBoxService.showMessage("error", ["Failed to generate UPI Payment QR Code."]);
        }
      }, err => {
        this.loadingUpiQr = false;
        this.msgBoxService.showMessage("error", ["Error communicating with UPI Gateway."]);
      });
  }
