...
export class SSU_PatientComponent {
  ...
  CheckValidation(): boolean {
    ...
    if (this.model.IsValid(undefined, undefined)) {
      ...
      let invalidControls = [];
      for (var name in this.model.SsuPatientValidator.controls) {
        if (this.model.SsuPatientValidator.controls[name].invalid) {
          invalidControls.push(name);
        }
      }
      this.msgBoxServ.showMessage("failed", ["One or more values are missing: " + invalidControls.join(", ") + ". Pls check and try again."]);
      return false;
    }
